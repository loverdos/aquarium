...
\section{Performance}
To evaluate the performance of Aquarium with respect to the requirements
set, 
...
Mbits/sec, as reported by the \texttt{iperf} utility.  No particular
...
{\sc dsl} to handle billing events for 5 types of resources, using 3 overloaded
pricelists, 2 overloaded algorithms, all of which were combined to 5 different
agreements. To drive the benchmark, we used a synthetic load generator that
produced billing events, at a configurable rate per minute.  


\begin{figure}[t]
    \begin{center}
        \includegraphics[scale=0.6]{perf.pdf}
    \end{center}
    \caption{Billing performance at 100 active users for a configurable number of
    messages per second.}
    \label{fig:perf}
\end{figure}
